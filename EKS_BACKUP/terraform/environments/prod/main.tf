...
terraform {
  ...
  backend "s3" {
    bucket = "hama-nest-wallet-terraform-state-prod"
...
# Addons Module (without ECR creation)
module "addons" {
...
# Helm releases for production environment
...
resource "helm_release" "argocd" {
  ...
  values = [
    yamlencode({
      global = {
        domain = "argocd-prod.${var.project_name}.local"
      }
      server = {
        service = {
          type = "LoadBalancer"
